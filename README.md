...
* **Node counts** (`RECOMMENDED_NODES`, from per-axis studies against directly computed held-out points; the default region is 2–200 / 4–100): NR `k=7 F0=3 V=4 p0=2 dp=3 q0=3 dq=3` (4,536 points); ER `F0=3 V=10 p0=4 dp=5 q0=3 dq=5` (9,000 points; **the p0/dp/q0/dq counts are provisional — they come from studies on the previous, narrower p0/q0 boxes and are being redone**; the ER band is sensitive to `V` through the Ep edge).  These target ~1e-7 worst-case error per axis, i.e. ~0.02 in the log-likelihood at 20,000 events (the error is `N_events × δN/N`).  Always confirm with `validate` — the per-axis studies cannot see cross terms.
