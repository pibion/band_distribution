...
* **Interpolant.** Tensor-product Chebyshev series through Chebyshev–Lobatto nodes, built and evaluated with `numpy.polynomial.chebyshev` (`chebfit`/`chebval`; exact at the nodes, ~40 µs per lookup natively — about 0.1% of a 20,000-event likelihood evaluation).  It **never extrapolates** — a query outside the table's box raises `OutOfBoxError`, so keep the MCMC prior inside the box (default box: `k` 0.13–0.22, `F0` 1e-5–1, `V` 2.7–3.3, `p0` 0.2–0.4, `p10-p0` 0–0.4, `q0` 0.04–0.08, `q10` 0.2–0.4; edit `DEFAULT_BOX` for a different one).
